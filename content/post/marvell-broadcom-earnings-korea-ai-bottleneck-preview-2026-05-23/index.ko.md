...
이번 이벤트의 본질은 **AI 맞춤형 칩 자체가 아니라 AI 시스템의 물리 병목**이다. AI 데이터센터가 커질수록 병목은 GPU 하나에서 HBM, 맞춤형 ASIC, 이더넷 스위치, 광연결, 고성능 패키지, 전력 안정화 부품, 고속기판, 테스트소켓으로 넓어진다.

브로드컴은 이 흐름의 가장 큰 숫자다. Q1 FY2026 매출은 **193.11억 달러**, AI 매출은 **84억 달러**, Q2 전체 매출 가이던스는 **220억 달러**, Q2 AI 반도체 매출 가이던스는 **107억 달러**다. 산식상 Q2 AI 반도체 매출은 전체 매출 가이던스의 **48.6%**다. 브로드컴은 더 이상 “반도체 + VMware”가 아니라 **AI 맞춤형 칩과 AI 네트워크에 VMware 현금흐름이 붙은 회사**로 봐야 한다. ([Broadcom][1])

마벨은 숫자는 작지만 신호는 더 날카롭다. Q4 FY2026 매출은 **22.19억 달러**, Q1 FY2027 가이던스는 **24.0억 달러 ±5%**다. Q4 데이터센터 매출은 **16.51억 달러**, 매출 비중 **74%**다. 마벨이 Q2 가이던스와 데이터센터 매출에서 재가속을 보여주면, 시장은 “AI 병목이 연산칩에서 연결·광·메모리 이동으로 내려오고 있다”는 쪽으로 다시 가격을 매길 수 있다. ([Marvell][2])

한국 투자자의 결론은 분명하다. 이 실적발표는 미국 반도체 두 종목만 보는 이벤트가 아니다. 한국 쪽에서는 **SK하이닉스·삼성전자 메모리**, **삼성전기 실리콘 커패시터/FC-BGA**, **고속 PCB와 테스트소켓**을 다시 점검하는 이벤트다.

다만 모든 한국 반도체가 같이 수혜를 받는 것은 아니다. 실제 고객 인증, 수주, 판가 상승, 수율, 마진이 확인된 병목만 살아남는다. “AI 수혜”라는 이름만 붙은 범용 PCB·광통신·소재주는 조심해야 한다.

한 줄 결론은 이렇다. **마벨·브로드컴 실적은 한국 반도체 투자에서 HBM 단일 베팅을 AI ASIC·네트워크·패키징·전력 안정화 병목으로 넓힐지 확인하는 시험대다.**
...
| 한국 시간 | **2026년 5월 28일 05:45 KST** | **2026년 6월 4일 06:00 KST** |
| 최근 기준 매출 | Q4 FY26 **22.19억 달러** | Q1 FY26 **193.11억 달러** |
| 다음 분기 가이던스 | Q1 FY27 **24.0억 달러 ±5%** | Q2 FY26 **220억 달러** |
| AI 핵심 숫자 | Q4 데이터센터 **16.51억 달러**, 비중 **74%** | Q1 AI 매출 **84억 달러**, Q2 AI 반도체 **107억 달러** 가이던스 |
| 핵심 질문 | 맞춤형 칩·광연결·CXL이 매출 가속으로 이어지는가 | AI ASIC과 AI 이더넷 네트워크가 107억 달러 기준선을 넘는가 |

계산은 단순하다. 마벨의 Q1 FY2027 가이던스 중간값은 직전 분기 대비 **+8.2%**다. 산식은 `24.0 / 22.187 - 1`이다. 브로드컴 Q2 AI 반도체 가이던스의 Q1 대비 증가율은 **+27.4%**다. 산식은 `10.7 / 8.4 - 1`이다. Q2 전체 매출 가이던스에서 AI 반도체가 차지하는 비중은 **48.6%**다. 산식은 `10.7 / 22.0`이다.
...
브로드컴은 이번 이벤트의 더 큰 신호다. 시장이 보고 싶은 것은 Q2 AI 반도체 매출 **107억 달러**를 넘는지뿐만이 아니다. 더 중요한 것은 **AI 네트워크 비중이 커지는지**다.
...
**브로드컴 콜에서 가장 강한 문장**은 이런 형태일 것이다.
...
마벨의 Q4 데이터센터 매출은 **16.51억 달러**, 전체의 **74%**다. 이제 마벨은 더 이상 범용 반도체 사이클주가 아니다. 데이터센터 AI 인프라 반도체 회사로 재분류된 상태다. ([Marvell][2])
...
다만 한국 투자자가 조심해야 할 점도 있다. 광 DSP와 실리콘 포토닉스의 초과 이익은 상당 부분 마벨·브로드컴 같은 미국 설계사와 대만·일본 패키징/소재 생태계에 귀속될 가능성이 크다. 한국에서 더 현실적인 연결은 **메모리, 패키지 기판, 전력 안정화 부품, 테스트소켓**이다.
...
첫째, FC-BGA는 AI ASIC·스위치 ASIC·네트워크 칩 패키징 수요와 직접 연결된다. 둘째, MLCC와 실리콘 커패시터는 AI 패키지의 전력 안정화와 연결된다. 셋째, 삼성전기는 이미 **약 1.5조원 규모 실리콘 커패시터 공급계약**을 공식 발표했다. 공급 기간은 2027년 1월 1일부터 2028년 12월 31일까지다. ([Samsung Electro-Mechanics][6])
...
- Q1 FY27 매출이 가이던스 중간값 **24억 달러**를 넘고, Q2 가이던스가 재가속을 보여준다.
...
브로드컴이 AI 반도체 107억 달러 기준선을 넘고 AI 네트워크 가시성을 높이면, 한국에서는 HBM뿐 아니라 FC-BGA, 고속 PCB, 테스트소켓, 실리콘 커패시터까지 다시 봐야 한다. 마벨이 custom silicon과 광연결에서 재가속을 보여주면, AI 서버는 GPU 박스가 아니라 **메모리·연결·패키징·전력 안정화 시스템**이라는 해석이 더 강해진다.
...
한 줄로 정리하면, **마벨·브로드컴 실적은 HBM 단일 베팅을 끝내는 이벤트가 아니라, HBM 위에 붙는 AI 네트워크·패키징·전력 안정화 병목을 확인하는 이벤트다.**
