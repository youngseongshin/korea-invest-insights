...
## 1. First — why the parts they make are different

### 1.1 SPG reducers — the robot joint's core

As explained in the [Korea robotics value-chain piece](/post/korea-robotics-value-chain-complete-map-2026-05-11/), reducers convert fast motor rotation into slow, strong torque. Every robotic joint (shoulder, elbow, wrist, hip, knee, ankle) uses one. A humanoid contains dozens of reducers.

```
Why reducers are expensive and hard:
...
```
SPG: the part that makes a robot move (joints = reducers)
...
| Robot share of backlog | Undisclosed (assumed small) | ₩12.4bn (1.1% of total) |
...
## 3. SPG — better robot asset, but a price problem
...
PER 110× embeds "Korean robot industry will explode and SPG's earnings will multiply by tens." That expectation has to deliver. If it doesn't, the multiple compresses sharply.

Specifically, precision robot reducers are still \~3% of SPG's revenue. The other 97% is industrial motors / reducers. The name "robot core-part play" is the marketing frame; the financials still read like an "industrial motor company."

### 3.3 Analytical position

Good company, not good price. The reducer-core thesis is real. But the price has substantially front-run the "global humanoid orders → earnings explosion" scenario. Multiple compression resolves through one of two paths: (a) SDD-actuator real sales cleanly clearing the 5,000-unit guidance, or (b) a major global humanoid order officially confirmed. Without one, PER 110× is "supported by expectation alone."

---

## 4. Halla Cast — less core, lighter price, still has verification
...
Halla Cast's end-2025 backlog is ₩1,134.6bn — large. But the composition is:
...
## 5. Side-by-side — which carries less risk
...
## 6. Fair value — what's adequate for each

### 6.1 SPG

| Scenario | Logic | Applied PER | Implied price range |
|---|---|---:|---|
| Current (over-anticipation) | Robot revenue 3%, multiple driven by expectation | 110× | ₩135,100 (now) |
| Reasonable entry | Confirmed global order + multiple compresses | 70-80× | \~₩86,000-₩98,000 |
| Sharp-decline interest | SDD volumes + major order delays | 50-60× | \~₩61,000-₩74,000 |

At ₩135,100 the price embeds a meaningful share of the "global humanoid order → revenue explosion" scenario. Multiple normalization requires fundamental evidence — SDD-actuator real volumes or a confirmed major global customer order.

### 6.2 Halla Cast

12-month forward EPS (average of 2026E and 2027F) ≈ ₩333 basis:

| Scenario | Applied PER | Implied price | Vs. current |
|---|---:|---:|---:|
| Bear (robot orders stall) | 35× | \~₩11,700 | -31% |
| Base (current order holds + base business grows) | 55× | \~₩18,300 | +8% |
| Bull (robot SKU expansion + mass production) | 70× | \~₩23,300 | +38% |

Current ₩16,900 sits slightly below base ₩18,300. If robot backlog expands to ₩50.0bn+, the bull-case ₩23,300 opens.
...
* SDD-actuator actual unit sales (target 5,000)
* Confirmed global humanoid customer orders (currently Rainbow / BD anchored)
* Precision robot-reducer revenue share (currently \~3% → potential 10%+)
* 2H 2026 results showing recognized robotics revenue

### 7.2 Halla Cast

* Robot-backlog evolution (₩12.4bn → ₩50.0bn+ would shift classification)
* \~10 additional SKUs under discussion converting to firm orders
* 2H 2026 robot mass-production revenue recognition
* Margin restoration (2025 6.6% → 2026 8%+)
* New plant yield / utilization stabilization

### 7.3 Joint

* Tesla Optimus mass-production timeline (Fremont line currently in "Construction")
* Samsung Electro-Mechanics 2H humanoid camera-module mass production → ecosystem-wide signal
* Hyundai Motor Group 3Q RMAC operation → Boston Dynamics Atlas supply-chain acceleration

---

## 8. Bottom line

SPG is the "crossroads" of Korea's robot value chain — the only supplier of reducers to Boston Dynamics, Rainbow Robotics, Samsung, and LG. But ₩3.0tn at PER 110× — for a company where robot revenue is still 3% — requires "everything to go right" for that price to feel justified.
...
*This article is for research and informational purposes only and does not constitute investment advice. SPG data references Samsung Securities, KB Securities, and Hana Securities reports. Halla Cast data references NH Investment & Securities, Korea Investment & Securities, and KB Securities reports. Prices reflect Maeil Business Newspaper data (May 12). Portions of SPG 2026 consensus and Halla Cast's actual customer identity are not publicly verified. The market's "global AI automaker = Tesla" inference is unofficial. Fair-value scenarios are analyst estimates and may be wrong. Korean robotics remains industry-early with uncertain mass-production timelines. Analysis can be wrong. Data cut: May 12, 2026 KST.*
