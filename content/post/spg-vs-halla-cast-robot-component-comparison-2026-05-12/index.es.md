...
## 1. Primero — por qué las piezas que fabrican son diferentes

### 1.1 Reductores de SPG — el núcleo de la articulación del robot

Como se explicó en el [análisis de la cadena de valor robótica](/post/korea-robotics-value-chain-complete-map-2026-05-11/), los reductores convierten la rotación rápida del motor en un par lento y potente. Cada articulación robótica (hombro, codo, muñeca, cadera, rodilla, tobillo) usa uno. Un humanoide contiene docenas de reductores.

```
Por qué los reductores son caros y difíciles de fabricar:
...
```
SPG: la pieza que hace moverse al robot (articulaciones = reductores)
...
## 2. Los números — qué tanta diferencia hay
...
| Cartera de pedidos robóticos | No divulgada (se asume pequeña) | ₩12,4 bn (1,1 % del total) |
...
## 3. SPG — mejor activo robótico, pero con un problema de precio
...
### 3.2 Por qué el precio es pesado
...
Un PER de 110× incorpora la expectativa de que "la industria robótica coreana va a explotar y los beneficios de SPG se multiplicarán por decenas". Esa expectativa tiene que cumplirse. Si no lo hace, el múltiplo se comprime con brusquedad.

En concreto, los reductores de precisión para robots siguen siendo \~3% de los ingresos de SPG. El 97% restante son motores y reductores industriales. El relato de "valor puro en piezas core robóticas" es un encuadre comercial; los estados financieros siguen leyéndose como los de una "empresa de motores industriales".

### 3.3 Posición analítica

Buena empresa, precio no tan bueno. La tesis de los reductores como pieza core es real. Pero el precio ya se ha adelantado de forma sustancial al escenario de "pedidos humanoides globales → explosión de beneficios". La normalización del múltiplo requiere evidencia fundamental — volúmenes reales del actuador SDD que superen claramente el objetivo de 5.000 unidades, o la confirmación oficial de un gran pedido global. Sin ello, PER 110× está "sostenido únicamente por expectativas".

---

## 4. Halla Cast — menos core, valoración más ligera, verificación pendiente
...
La cartera de pedidos de Halla Cast a finales de 2025 es de ₩1.134,6 bn — significativa. Pero su composición es:
...
## 5. Cara a cara — cuál asume menos riesgo
...
## 6. Valor razonable — qué nivel es adecuado para cada una

### 6.1 SPG

| Escenario | Lógica | PER aplicado | Rango de precio implícito |
|---|---|---:|---|
| Actual (sobreanticipación) | Ingresos robóticos 3%, múltiplo impulsado por expectativas | 110× | ₩135.100 (actual) |
| Entrada razonable | Pedido global confirmado + compresión del múltiplo | 70-80× | \~₩86.000-₩98.000 |
| Zona de interés en caída pronunciada | Volúmenes SDD + retrasos en pedidos mayores | 50-60× | \~₩61.000-₩74.000 |

A ₩135.100 el precio incorpora una parte significativa del escenario de "pedido humanoide global → explosión de ingresos". La normalización del múltiplo requiere evidencia fundamental — volúmenes reales del actuador SDD o la confirmación de un pedido mayor relevante de un cliente global.

### 6.2 Halla Cast

BPA forward a 12 meses (media de 2026E y 2027F) ≈ base ₩333:

| Escenario | PER aplicado | Precio implícito | Vs. actual |
|---|---:|---:|---:|
| Bajista (pedidos robóticos se estancan) | 35× | \~₩11.700 | -31% |
| Base (pedido actual se mantiene + negocio base crece) | 55× | \~₩18.300 | +8% |
| Alcista (expansión de referencias robóticas + producción en serie) | 70× | \~₩23.300 | +38% |

El precio actual de ₩16.900 está ligeramente por debajo del escenario base de ₩18.300. Si la cartera robótica se expande hasta ₩50,0 bn+, el caso alcista de ₩23.300 se abre.
...
* Ventas unitarias reales del actuador SDD (objetivo 5.000)
* Pedidos confirmados de clientes humanoides globales (actualmente anclados en Rainbow / BD)
* Cuota de ingresos de reductores de precisión para robots (actualmente \~3% → potencial 10%+)
* Resultados del 2S 2026 que muestren ingresos robóticos reconocidos

### 7.2 Halla Cast

* Evolución de la cartera robótica (₩12,4 bn → ₩50,0 bn+ cambiaría la clasificación)
* Conversión de las \~10 referencias adicionales en discusión a pedidos en firme
* Reconocimiento de ingresos de producción en serie robótica en el 2S 2026
* Restauración de márgenes (6,6% en 2025 → 8%+ en 2026)
* Estabilización del rendimiento / utilización de la nueva planta

### 7.3 Conjunto del sector

* Calendario de producción en serie del Tesla Optimus (línea de Fremont actualmente en "Construcción")
* Producción en serie del módulo de cámara humanoide de Samsung Electro-Mechanics en 2S → señal para todo el ecosistema
* Operación del RMAC de Hyundai Motor Group en 3T → aceleración de la cadena de suministro del Boston Dynamics Atlas

---

## 8. Conclusión

SPG es el "nudo central" de la cadena de valor robótica de Corea — el único proveedor de reductores para Boston Dynamics, Rainbow Robotics, Samsung y LG. Pero ₩3,0 bn a PER 110× — para una empresa donde los ingresos robóticos siguen siendo el 3% — exige que "todo salga bien" para que ese precio se justifique.
...
*Este artículo tiene únicamente fines de investigación e informativos y no constituye asesoramiento de inversión. Los datos de SPG hacen referencia a informes de Samsung Securities, KB Securities y Hana Securities. Los datos de Halla Cast hacen referencia a informes de NH Investment & Securities, Korea Investment & Securities y KB Securities. Los precios recogen datos de Maeil Business Newspaper (12 de mayo). Partes del consenso 2026 de SPG y la identidad real del cliente de Halla Cast no están verificadas públicamente. La inferencia del mercado de "fabricante global de IA = Tesla" no es oficial. Los escenarios de valor razonable son estimaciones de analistas y pueden ser erróneas. La robótica coreana sigue siendo una industria en etapa temprana con calendarios de producción en serie inciertos. El análisis puede estar equivocado. Datos a: 12 de mayo de 2026 KST.*
