...
## 1. Conclusion d'Abord — La Série Atteint le Chapitre « Transit »

### 1.1 Panorama Après l'Établissement des Trois Pics

La série a débuté avec trois pics :

| Série | Société | Identité du modèle | Coût implicite des fonds propres |
| --- | --- | --- | ---: |
| Partie 1 | Meritz Financial Holdings | Capitalisation-rachat composé (statique) | 11,5 % |
...
| Indicateur | Valeur |
| --- | ---: |
| Clôture du 30 avril 2026 | ₩99 900 |
...
- Coût implicite des fonds propres = 11,9 / 0,78 = 15,26 % ✓
...
## 2. NVR +18 % — Le Volume a Reconnu le Transit en Premier

### 2.1 OBV vs NVR — Différentes Qualités d'Information

Shinhan a été détectée via un screener d'accumulation interne utilisant les signaux OBV et NVR. Les deux indicateurs semblent similaires mais véhiculent des qualités d'information différentes.

```
Taux de variation OBV brut = (OBV actuel − OBV passé) / OBV passé
NVR = (volume journées hausse − volume journées baisse) / (volume journées hausse + volume journées baisse)
```

L'OBV est sensible au dénominateur. Un OBV de départ faible gonfle le taux de variation. Le NVR, normalisé entre -1 et +1, permet une comparaison plus propre entre différentes valeurs.

### 2.2 Ce qui s'est Passé sur le Carnet d'Ordres de Shinhan
...
Rétro-ingénierie du NVR +18 % en volumes directionnels :

```
Volume net directionnel = 6,3 M
NVR = 18 %
...
Vérification :
- Hausse + Baisse = 20,65 + 14,35 = 35,0 M ✓
...
Pour évaluer équitablement la crédibilité du signal, une mise en garde doit être explicitement signalée. Le libellé interne « 30 jours de trading en arrière (19/2) → 30/4 » ne correspond pas au calendrier. Du 19 février au 30 avril 2026, c'est environ 51 jours ouvrés avant même d'exclure les jours fériés du marché coréen — clairement plus de 30 jours de trading.
...
## 3. Les Fondamentaux Montrent la Coordonnée de Transit
...
Le cœur du Value-up 2.0 n'est pas une hausse de dividende en titre. Selon la communication de valeur d'entreprise de Shinhan au KRX :

| Indicateur | Direction annoncée |
| --- | --- |
| ROE | 10 %+, géré dans une fourchette 10–12 % |
| Taux de distribution | 50 %+, piloté par formule |
...
## 4. Localiser Shinhan via le Coût Implicite des Fonds Propres

### 4.1 Dans la Matrice de la Série

Regroupant toutes les lectures de coût implicite des fonds propres produites par la série dans un seul tableau :

| Société | ROE | PBR | Coût implicite des fonds propres |
| --- | ---: | ---: | ---: |
...
### 4.2 Scénarios de PBR — Le Mouvement de Coordonnée Visualisé

En maintenant le ROE à 11,9 % constant et en abaissant progressivement le coût implicite des fonds propres, la fourchette de prix s'élargit :
...
La lecture de l'arithmétique du fonctionnement du transit de coordonnée en clarifie davantage le sens :
...
## 5. Volume et Fondamentaux en Alignement

### 5.1 Trois Signaux, Même Direction
...
Ce changement de phase est lui-même un signal de stade plus profond dans la progression du changement de reconnaissance au sein de la finance coréenne.

---

## 6. Deux Mises en Garde Honnêtes

### 6.1 Les Résultats Hors Banque Nécessitent une Vérification Structurelle
...
## 7. La Prochaine Étape de Vérification — Signaux qui Mesurent la Vitesse du Transit

Non pas des déclencheurs de trading. Des points d'observation montrant la vitesse à laquelle le « transit entre les sommets » progresse.
...
La finance coréenne a évolué d'un stade plus profond dans son histoire de réévaluation — passant du « paysage à trois pics » au « transit entre les sommets commençant à être visible ». Et les données qui suivent ce transit s'impriment automatiquement chaque trimestre. C'est en soi une raison suffisante pour que cette série continue.

Le prochain article de la série paraîtra lorsque (1) le ROE 2T26 de Shinhan sera vérifié, (2) la vitesse à laquelle l'écart de coût implicite des fonds propres se resserre de 15,26 % vers 13 % deviendra observable, et (3) les autres cas de suiveurs — Hana Financial, DB Insurance, Korea Investment Holdings — commenceront à imprimer.
...
*Cet article est destiné à la recherche et au commentaire uniquement, et ne constitue pas un conseil en investissement. Les scénarios ROE / CET1 / taux de distribution / PBR / NVR sont fondés sur des niveaux publiés, des estimations sell-side (WiseReport, Mirae Asset Securities, etc.), des supports IR des sociétés et des divulgations KRX. Les NVR / OBV sont calculés à partir de données internes ; la précision de la définition de fenêtre nécessite une vérification séparée. Les tickers cités sont illustratifs pour le cadre d'analyse, et non des recommandations. Effectuez votre propre diligence raisonnable et consultez des conseillers agréés avant toute décision d'investissement.*
