...
> 📚 Serie Pearl Abyss
> Cobertura previa: recepción post-lanzamiento, resumen de resultados del 1T26, estructura de reconocimiento de ingresos de Crimson Desert

*Pearl Abyss publicó un 1T26 récord: ingresos de KRW 328.500M, beneficio operativo de KRW 212.100M. Sin embargo, la acción ha mostrado debilidad. En la misma llamada de resultados, la compañía señaló que está "explorando la próxima fase de expansión, incluido el DLC" para Crimson Desert. Medios extranjeros lo interpretaron como "DLC confirmado", el mercado reaccionó y luego volvió a caer. El significado de ese comentario sobre DLC está siendo ampliamente malinterpretado. El titular no es "KRW 60.000M en ingresos por DLC". El titular es que ese comentario elimina el "descuento por el abismo de 2027" que el mercado ha estado aplicando a Pearl Abyss. Eso mueve el múltiplo desde un PER de un solo dígito hacia la media global de publishers de videojuegos (15–25x) — y esa reclasificación importa mucho más que los ingresos en sí.*
...
## 1. Cómo ve el mercado a Pearl Abyss hoy

### 1.1 Por qué un trimestre récord aun así generó ventas
...
### 1.2 Qué teme el mercado — el "precipicio del juego de paquete"

```
Patrón clásico de ingresos de un juego de paquete:
...
### 1.3 Qué implica realmente un PER de un solo dígito

```
PER aplicado actual a Pearl Abyss:
→ Un solo dígito (\~8x)
...
## 2. Qué dice realmente el comentario sobre DLC — no es una confirmación

### 2.1 Qué dijo realmente la dirección

```
De la carta de resultados del 1T26:
...
### 2.2 Incluso "explorando" tiene peso

```
Qué significa que una compañía incluya "explorando DLC"
...
### 2.3 Por qué el mercado no reaccionó más

```
Razón 1: Presión macro global
...
## 3. Qué crea realmente el DLC — ingresos directos vs. cambio de múltiplo

### 3.1 El efecto directo sobre ingresos (el menor)
...
Tabla de escenarios para ingresos incrementales:
...
```
Escenario base (ingresos DLC KRW 55–75.000M, beneficio op. KRW 35–50.000M):
...
### 3.3 El efecto por cambio de múltiplo (el mayor)
...
### 3.4 Por qué entender esta diferencia importa
...
## 4. Valores razonables por escenario

### 4.1 Escenarios sobre el PER aplicado

```
Escenario A (bajista — sigue siendo un juego de paquete):
...
### 4.2 Dónde se sitúa el precio — asimetría

```
La cotización actual se acerca al escenario A.
...
## 5. Los próximos 6 meses — qué vigilar

### 5.1 Qué tiene que demostrar la compañía

```
Señales necesarias para la transición A → B:
...
### 5.2 Qué tiene que demostrar el propio juego
...
### 5.3 El contexto macro

```
La barrera macro del post anterior:
...
## 6. Decisión de trading — tenedores vs. nuevos compradores

### 6.1 Si ya tienes posición
...
### 6.2 Si eres un nuevo comprador
...
### 6.3 La lectura central — "esta es una ventana de 'la compañía debe jugar una carta'"

```
La esencia de Pearl Abyss ahora mismo:
...
## 7. Cómo conecta esto con otros posts

```
Post de síntesis del ciclo macro:
...
*Este artículo es investigación y comentario únicamente y no constituye asesoramiento de inversión. Las cifras del 1T26 de Pearl Abyss y la guía FY2026 corresponden a la carta de resultados oficial de la compañía. El tratamiento de reconocimiento de ingresos de Crimson Desert (PC neto de impuestos; consola neta de comisiones de plataforma) se corresponde con la divulgación de la compañía. La frase "explorando diversas vías, incluido el DLC" es textual de la carta de resultados del 1T; la palabra "confirmado" no se utiliza. La estimación de \~5,8M de unidades acumuladas está inferida a partir de los datos de reseñas de Steam; el hito oficial es 5M al 15 de abril. Los escenarios de ingresos por DLC (base instalada, tasa de adopción, precio) son estimaciones del autor y no están confirmados. El PER de un solo dígito aplicado es una estimación basada en el precio del 15 de mayo y el BPA consenso estimado para 2026E. Los rangos de PER por escenario de 6–18x son estimaciones del autor y no están garantizados. Las variables macro globales (tipos de interés en EE. UU., precio del petróleo, divisas, VIX) pueden mover la acción de forma independiente. El análisis puede estar equivocado. Fecha de corte de datos: 15 de mayo de 2026 KST.*
