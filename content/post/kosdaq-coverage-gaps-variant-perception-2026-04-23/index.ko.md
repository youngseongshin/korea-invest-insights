...
**엔터 커버리지는 비정상적으로 두텁다.** SM은 8개사·14건, YG는 7개사·12건으로 중형 인덱스 종목 수준의 밀도다. 같은 섹터 내에서 <strong>SAMG엔터테인먼트(2개사·2건)</strong>는 "준(準) 커버리지 공백" 후보다 — 프레임워크를 갖출 만큼은 커버됐지만, 히트 IP 테제가 터졌을 때 리레이팅 여력이 충분하다.
...
**방산 소형주에는 실제 트래픽이 몰리고 있다.** 시총 1,000억\~3,000억 원대에 3개 종목이 포함됐으며, 모두 최근 리포트(4월 7일, 15일, 17일)가 있다. <strong>코츠테크놀로지(시총 1,240억 원, 3개 증권사)</strong>의 시총 대비 커버리지 밀도는 통상 셀사이드 컨센서스 편입 직전에 나타나는 패턴이다.
