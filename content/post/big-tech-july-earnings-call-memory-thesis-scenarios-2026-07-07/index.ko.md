...
아래 표는 개인 포트폴리오 지시가 아니라 공개용 판단 프레임이다.

| 종목 | 위치 | 공개용 판단 |
|---|---|---|
| 삼성전자 | 한국 메모리 core | 유지. 조건 충족 시에만 추가 검토 |
| SK하이닉스 | HBM pure beta | 대기. 기존 진입 조건 유지, 콜 결과는 목표 비중 계획에만 반영 |
| 마이크론 | 미국 메모리 계약 구조 벤치마크 | 관찰 목록 등재 |
| 마벨 | custom silicon·Trainium 파급효과 | 메모리 논리와 분리해서 관리 |
| 메타 | AI capex 수요자이자 클라우드 수익화 후보 | 7월 29일 뉴스 소화 리스크 별도 관리 |
...
판단은 유지, 추가는 조건부다. 다음 세 가지가 모두 확인될 때만 추가 검토가 맞다.

1. 7월 30일 실적 콜에서 DS 본업 이익이 시장 기대 이상이라는 점.
2. 7월 말 빅테크 콜 네 건 중 세 건 이상이 강화 판정이라는 점.
3. TrendForce 3분기 가격 상승률 전망이 두 자릿수를 유지한다는 점.

5월 8일 종가 26만 8,500원과 목표가 50만원, 1년 +490%라는 좌표는 참고할 수 있다. 다만 4월에 세웠던 19만원 이하 매수 선호 기준은 가격이 이미 지나가 무효다.

### 5.3 SK하이닉스

한 줄 논리는 이렇다. SK하이닉스는 HBM 사이클의 가장 직접적인 수혜주다. 여기에 가격 상한 없는 장기계약 보도가 사실이면, 바닥은 깔고 천장은 열어둔 구조로 남은 가격 상승분을 가장 온전히 가져갈 수 있다.

판단은 대기다. 기존 진입 조건, 즉 두바이유 실물 프리미엄 배럴당 15달러 미만과 원달러 1,470원 미만의 동시 충족을 유지한다. 현재 환율 환경에서는 조건이 멀다. 콜 결과는 진입 조건이 아니라 조건 충족 시 목표 비중에 반영한다.
...
8월 ADR 상장은 자금 유입과 물량 부담이 함께 걸린 보조 변수다. 논리의 축은 아니다.

### 5.4 마이크론

마이크론은 계약 구조 전환의 원조다. 매출의 상당 부분이 바닥 가격이 깔린 5년 계약으로 잠기는 회사이고, 미국 상장 메모리 순수주라 접근성이 좋다.

판단은 관찰 목록 등재다. 진입 검토 조건은 두 가지다.

1. 7월 말 빅테크 콜에서 세 회사 이상이 강화 판정을 받을 것.
2. SCA 구조, 즉 take-or-pay와 바닥 가격이 훼손되지 않는다는 점이 유지될 것.

상한이 있는 계약이라 가격 추가 상승분은 SK하이닉스보다 덜 가져갈 수 있다. 또 이미 FY3Q26 실적 발표에 주가가 크게 반응했다는 점도 함께 봐야 한다. 폐기 조건은 계약 재협상 보도, 가격의 분기 대비 하락 전환, 공급 과잉 발언의 동시 출현이다.

### 5.5 마벨

마벨은 메모리 논리가 아니라 아마존 Trainium과 custom silicon 논리다. Trainium 매출 약정과 세대별 선예약은 강력한 배경이지만, 3세대와 4세대에서 마벨의 설계 몫이 얼마인지는 검증되지 않았다. 따라서 7월 30일 아마존 콜의 Trainium 발언을 마벨 호재로 바로 번역하면 안 된다. 설계 몫 확인 전까지 절반만 반영한다.

### 5.6 메타

메타는 AI capex 수요자이면서 동시에 클라우드 수익화 후보가 됐다. 7월 29일 콜에서 클라우드 사업이 공식화되지 않거나 2027년 언급이 없으면, 7월 초 보도 이후 반등분이 되돌려질 수 있다. 다만 이것은 메타 주가 이벤트이지 메모리 논리 이벤트가 아니다. 분리해서 봐야 한다.
