...
## 1. Bottom Line First — The Series Reaches the "Coordinate-Definition Event" Chapter

### 1.1 Evolution of the Series' Analytical Dimensions

A summary of how the series' analytical dimensions have evolved:

| Series | Company | Analytical dimension | Coordinate state | Tone |
| --- | --- | --- | --- | --- |
| Parts 1–3 | Meritz / Kiwoom / KB | Peak landscape | Settled | Constructive |
...
| Item | Value |
| --- | ---: |
| Viva Republica (parent, unlisted) | — |
| 2025 consolidated revenue | ₩2,698.3B (+38.0% YoY) |
...
- Merchant share = 922.8 / 2,698.3 = 34.20% ≈ 34.2% ✓
- Toss Securities OPM = 445.8 / 882.6 = 50.51% ≈ 50.5% ✓
- Toss Securities attributable = 333.9 × 0.974 = ₩325.2B ✓
...
## 2. Anatomy of "Quality of Earnings" — A Neutral Decomposition
...
Verification: Toss Securities attributable − Group NI = 325.2 − 201.8 = ₩123.4B

This decomposition isn't constructive or bearish — it's just fact. The judgment of earnings quality applies differently across IPO scenarios — at the lower price tier, "Toss Securities dependence = cyclical discount"; at the higher price tier, "Toss Securities' strong margin = a verified monetization case for the super-app model."

### 2.2 Toss Securities — Two Sides of Cyclicality

Toss Securities' OPM 50.5% is higher than Kiwoom Securities (Part 2). How the market interprets this is the central variable for the IPO price.
...
| Item | Toss Bank 2025 | KakaoBank 2025 (Series Part 6) |
| --- | ---: | ---: |
...
### 2.4 Viva Republica Standalone — Where the App Body Sits
...
As of May 3, 2026, no publicly available F-1 or ADR-related public registration filing has been confirmed. Per SEC rules, a Foreign Private Issuer (FPI) may submit a non-public draft registration statement, which must be made public at least 15 days before commencing the road show or 15 days before the targeted effective date.
...
Viva Republica likely lists as an ADR while maintaining its Korean entity. ADR structure means:

- Liquidity constraints driven by the ADR ratio
- Information asymmetry vs US-domestic issuers under FPI reporting (no quarterly reporting requirement, etc.)
...
Toss operates financial businesses. To US investors, Korean financial-regulation exposure means:

- Direct exposure to FSC / FSS regulatory shifts
- Korean financial-policy changes affecting fintech multiples directly (as seen with KakaoBank)
...
Viva Republica has many shareholders (Ant Group, VCs, institutions, employee SBC, etc.). Per US IPO structure:

- Only registered shares list and trade first; secondary-share sales require separate SEC registration and company coordination
- Existing-shareholder lock-up negotiations are complex → post-IPO overhang concerns
...
Combining these five walls produces the natural lower bound:

```
SOTP fair-value midpoint (per source-2 base case): \~$7–10B
...
### 4.3 Implied Cost of Equity (earnings yield = 1/P/E, growth=0)
...
### 4.4 Gordon-Growth Adjustment (the perpetual-growth rate the market assumes)

Assuming cost of equity 9%:

```
$5B  justified g = 9% − 1/36.5  = 9% − 2.74% = 6.26%
$7B  justified g = 9% − 1/51.0  = 9% − 1.96% = 7.04%
$10B justified g = 9% − 1/72.9  = 9% − 1.37% = 7.63%
$15B justified g = 9% − 1/109.4 = 9% − 0.91% = 8.09%
```

Implication of this math:

| Price | Market-assumed perpetual growth | Multiple of Korea GDP growth (2–3%) |
| --- | ---: | ---: |
| $5B | 6.26% | \~2.5× |
| $7B | 7.04% | \~2.8× |
| $10B | 7.63% | \~3.1× |
...
Putting the six existing series companies plus Toss's four points on one matrix:

```
Implied cost-of-equity matrix (lower ← → higher):
...
## 5. The Same Company Could Land at Either Endpoint — Market Classification Sets the Price
...
The meta-message this fact adds to the series:

```
Series analytical-dimension evolution:
...
## 6. Seven Variables That Determine the Coordinate

Decomposing the price-determination process into seven accounting variables:

### 6.1 The Seven Price-Determining Variables

| Variable | Direction | Scenario sensitivity |
| --- | --- | --- |
| 1. Public F-1 timing | Late = price range itself drops | If unfiled by May–June, Q2 IPO becomes difficult |
| 2. US-market classification | "Super app" = upper region; "financial holding" = lower region | Determines $5B ↔ $15B endpoint choice |
| 3. Toss Securities OPM persistence | Repeatable = upper; cyclical = lower | OPM 50% → 35% regression worth approximately −$3–4B |
| 4. Advertising / brokerage growth | Ad +25% / brokerage +5% sustained = neutral | Ad slowdown = lower-end pressure |
| 5. Viva Republica standalone profitability | 1H26 turnaround = upper; sustained loss = lower | $1–2B impact |
| 6. Toss Bank separate-IPO signal | Strong signal = wider SOTP discount | $0.5–1B impact |
| 7. ADR structure + secondary-share liquidity | Large secondary offering = overhang concern | $1–2B impact |

### 6.2 Direction-of-Impact Summary
...
| Price | V1 | V2 | V3 | V4 | V5 | V6 | V7 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| $5B | Late | Financial holding | Cyclical | Slowing | Sustained loss | Strong signal | Large overhang |
| $7B | Timely | Mixed | Partial cyclical | Ad-only acceleration | Loss narrowing | Undecided | Partial overhang |
...
For analytical credibility, two honest caveats:

### 7.1 Two Interpretations of "Toss Securities OPM 50.5%"

Toss Securities OPM 50.5% is hard to evaluate externally in isolation. Determining whether it's "repeatable" or "cyclical" requires:

- Revenue decomposition (commissions / FX / cash / margin / other)
- Quarterly OPM trajectory
...
Market environment itself is a major IPO-pricing variable. The 2026 US IPO market isn't "shut" but isn't "fully open" either:

- Volatility: some large-IPO postponements
- Software-sector softness: pressure on fintech multiples
- Geopolitical risk: potential added discount on Korean ADRs
- Rate environment: function of fintech valuation multiples

These variables can swing price ±20%+ regardless of company fundamentals. Same fundamentals can land at the lower end or the upper end depending on the market environment when the IPO actually prices. That's another part of the essence of "pre-listing coordinate-definition event."

---

## 8. The Next Verification Step — Key Milestones in the IPO Calendar

Not trade triggers. Observation points showing how the "coordinate-definition event" progresses:

### 8.1 Schedule Milestones

| Timing | Event | Scenario impact |
| --- | --- | --- |
| Mid-May 2026 | KakaoBank, Korea Investment Holdings 1Q26 results | Korea fintech comparables price formation |
| Late May–June 2026 | Whether public F-1 is filed | Unfiled = Q2 IPO difficult → lower-range pressure |
| June 2026 | If F-1 publishes, segment disclosure verification | Variables 3, 4, 5 close accountingly |
| July 2026 | Roadshow start | Variable 2 (market classification) decision stage |
| August 2026 onward | Price negotiation → offer-price finalization | Seven variables aggregate into price |

### 8.2 Seven Critical Items in F-1/S-1

| Item | Variable closed |
| --- | --- |
| Toss Securities revenue decomposition + quarterly OPM trajectory | Variable 3 (cyclicality vs structurality) |
| Viva Republica standalone contribution margin | Variable 5 (app-body profitability path) |
| Advertising take-rate, customer cohorts, repeatability | Variable 4 (quality of advertising acceleration) |
| Brokerage by category growth rates | Variable 4 (brokerage slowdown cause) |
| Toss Payments TPV / EBITDA margin | PG-segment decomposition |
| Toss Bank separate-IPO plan disclosure | Variable 6 |
| ADR ratio + secondary-share share + lock-up terms | Variable 7 |
...
*This post is research and commentary only, not investment advice. The IPO valuation four-point analysis, implied cost of equity, SOTP estimates, the five institutional walls of US listing, and discount estimates are based on external reporting (Reuters, Yonhap, The Bell, Investing Korea) and company IR materials, and the analyst's inference based on Korea Investors Service analyses; actual IPO price, market classification, and forward results may differ. The four price points are tools showing the "possible range" only — no single point is treated as a "base case," and refusing to do so is the analytical principle of this piece. Gordon Growth Model assumption (cost of equity 9%) is a conservative anchor; actual values may vary. Tickers cited are illustrative for the framework, not recommendations. Do your own due diligence and consult licensed advisors before any investment decision.*
