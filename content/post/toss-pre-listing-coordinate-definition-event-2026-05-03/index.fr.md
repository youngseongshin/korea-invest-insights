...
## 1. La conclusion d'abord — La série atteint le chapitre de l'« Événement de définition de coordonnée »

### 1.1 Évolution des dimensions analytiques de la série

Résumé de l'évolution des dimensions analytiques de la série :

| Série | Société | Dimension analytique | État de la coordonnée | Ton |
| --- | --- | --- | --- | --- |
| Parties 1–3 | Meritz / Kiwoom / KB | Paysage de sommet | Établi | Constructif |
...
| Élément | Valeur |
| --- | ---: |
| Viva Republica (maison mère, non cotée) | — |
| Chiffre d'affaires consolidé 2025 | ₩2 698,3 Mds (+38,0 % en glissement annuel) |
...
- Part services marchands = 922,8 / 2 698,3 = 34,20 % ≈ 34,2 % ✓
- Marge opérationnelle Toss Securities = 445,8 / 882,6 = 50,51 % ≈ 50,5 % ✓
- Quote-part Toss Securities = 333,9 × 0,974 = ₩325,2 Mds ✓
...
## 2. Anatomie de la « qualité des bénéfices » — Décomposition neutre
...
Vérification : quote-part Toss Securities − résultat net groupe = 325,2 − 201,8 = ₩123,4 Mds

Cette décomposition n'est ni constructive ni baissière — ce sont simplement des faits. Le jugement sur la qualité des bénéfices s'applique différemment selon les scénarios d'introduction : au niveau de prix bas, « la dépendance à Toss Securities = décote cyclique » ; au niveau de prix élevé, « la marge solide de Toss Securities = un cas de monétisation du modèle super-application vérifié ».

### 2.2 Toss Securities — Les deux faces de la cyclicité

La marge opérationnelle de 50,5 % de Toss Securities est supérieure à celle de Kiwoom Securities (Partie 2). L'interprétation que le marché en fera est la variable centrale pour le prix d'introduction.
...
| Élément | Toss Bank 2025 | KakaoBank 2025 (Partie 6 de la série) |
| --- | ---: | ---: |
...
### 2.4 Viva Republica standalone — La situation du corps applicatif
...
Au 3 mai 2026, aucun dépôt public d'un F-1 ou d'un ADR n'a été confirmé. Selon les règles SEC, un émetteur étranger privé (FPI) peut déposer un projet de document d'enregistrement de manière non publique, qui doit être rendu public au moins 15 jours avant le début du road show ou 15 jours avant la date d'entrée en vigueur ciblée.
...
Viva Republica se cotera vraisemblablement sous forme d'ADR tout en conservant son entité coréenne. La structure ADR implique :

- Des contraintes de liquidité liées au ratio ADR
- Une asymétrie d'information par rapport aux émetteurs domestiques américains sous le régime FPI (pas d'obligation de rapports trimestriels, etc.)
...
Toss opère dans des activités financières. Pour les investisseurs américains, l'exposition à la réglementation financière coréenne signifie :

- Exposition directe aux évolutions réglementaires de la FSC / FSS
- Les changements de politique financière coréenne affectent directement les multiples des fintechs (comme observé avec KakaoBank)
...
Viva Republica compte de nombreux actionnaires (Ant Group, capital-risqueurs, institutionnels, SBC pour les salariés, etc.). Selon la structure des introductions américaines :

- Seules les actions enregistrées sont cotées et échangées en premier ; les ventes d'actions secondaires nécessitent un enregistrement SEC séparé et la coordination de la société
- Les négociations sur le lock-up des actionnaires existants sont complexes → risque d'excédent d'offre post-introduction
...
La combinaison de ces cinq obstacles produit la borne inférieure naturelle :

```
Valeur intrinsèque SOTP (scénario de base source-2) : \~$7–10 Mds
...
```
Résultat net consolidé 2025 : ₩201,8 Mds
...
Vérifications :
- $5B × 1 471 = ₩7 355 Mds ≈ ₩7,36 Bn ✓
- $7B × 1 471 = ₩10 297 Mds ≈ ₩10,30 Bn ✓
- 7 360 / 201,8 = 36,47 → 36,47× ≈ 36,5× ✓
- 10 300 / 201,8 = 51,04 → 51,04× ≈ 51,0× ✓
- 14 710 / 201,8 = 72,89 → 72,89× ≈ 72,9× ✓
- 22 070 / 201,8 = 109,37 → 109,37× ≈ 109,4× ✓

### 4.3 Coût implicite des capitaux propres (rendement bénéficiaire = 1/P/E, croissance=0)
...
### 4.4 Ajustement Gordon-Growth (le taux de croissance perpétuelle assumé par le marché)

En supposant un coût des capitaux propres de 9 % :

```
$5B  g justifié = 9 % − 1/36,5  = 9 % − 2,74 % = 6,26 %
$7B  g justifié = 9 % − 1/51,0  = 9 % − 1,96 % = 7,04 %
$10B g justifié = 9 % − 1/72,9  = 9 % − 1,37 % = 7,63 %
$15B g justifié = 9 % − 1/109,4 = 9 % − 0,91 % = 8,09 %
```

Implication de ce calcul :

| Prix | Croissance perpétuelle assumée par le marché | Multiple de la croissance du PIB coréen (2–3 %) |
| --- | ---: | ---: |
| $5B | 6,26 % | \~2,5× |
| $7B | 7,04 % | \~2,8× |
| $10B | 7,63 % | \~3,1× |
...
Représentation sur une seule matrice des six sociétés existantes de la série et des quatre points de Toss :

```
Matrice du coût implicite des capitaux propres (bas ← → élevé) :
...
## 5. La même société pourrait atterrir à l'une ou l'autre extrémité — La classification de marché fixe le prix
...
Le méta-message qu'ajoute ce fait à la série :

```
Évolution des dimensions analytiques de la série :
...
## 6. Sept variables qui déterminent la coordonnée

Décomposition du processus de détermination du prix en sept variables comptables :

### 6.1 Les sept variables déterminantes du prix

| Variable | Direction | Sensibilité au scénario |
| --- | --- | --- |
| 1. Calendrier du F-1 public | Retard = la fourchette de prix elle-même s'abaisse | Non déposé avant mai–juin → introduction T2 difficile |
| 2. Classification du marché américain | « Super app » = zone haute ; « holding financier » = zone basse | Détermine le choix d'extrémité $5B ↔ $15B |
| 3. Persistance de la marge opérationnelle Toss Securities | Récurrente = haute ; cyclique = basse | Régression marge opérationnelle 50 % → 35 % vaut environ −$3–4 Mds |
| 4. Croissance publicité / courtage | Pub +25 % / courtage +5 % soutenu = neutre | Ralentissement pub = pression vers le bas |
| 5. Rentabilité standalone Viva Republica | Redressement 1S26 = haute ; perte persistante = basse | Impact $1–2 Mds |
| 6. Signal d'introduction séparée Toss Bank | Signal fort = décote SOTP plus large | Impact $0,5–1 Md |
| 7. Structure ADR + liquidité actions secondaires | Offre secondaire importante = risque d'excédent d'offre | Impact $1–2 Mds |

### 6.2 Synthèse du sens d'impact
...
| Prix | V1 | V2 | V3 | V4 | V5 | V6 | V7 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| $5B | Retard | Holding financier | Cyclique | Ralentissement | Perte persistante | Signal fort | Excédent important |
| $7B | Ponctuel | Mixte | Partiellement cyclique | Accélération pub seule | Perte se réduisant | Indécis | Excédent partiel |
...
Pour la crédibilité analytique, deux mises en garde honnêtes :

### 7.1 Deux interprétations de « la marge opérationnelle de Toss Securities à 50,5 % »

La marge opérationnelle de Toss Securities à 50,5 % est difficile à évaluer de l'extérieur en isolation. Déterminer si elle est « récurrente » ou « cyclique » nécessite :

- La décomposition des revenus (commissions / change / trésorerie / marge / autres)
- La trajectoire trimestrielle de la marge opérationnelle
...
L'environnement de marché est lui-même une variable majeure de la fixation du prix d'introduction. Le marché américain des introductions 2026 n'est pas « fermé » mais n'est pas non plus « pleinement ouvert » :

- Volatilité : plusieurs reports d'introductions importantes
- Faiblesse du secteur logiciel : pression sur les multiples des fintechs
- Risque géopolitique : décote potentielle supplémentaire sur les ADR coréens
- Environnement de taux : variable déterminante des multiples de valorisation fintech

Ces variables peuvent faire varier le prix de ±20 %+ indépendamment des fondamentaux de la société. Les mêmes fondamentaux peuvent atterrir à l'extrémité basse ou haute selon l'environnement de marché au moment de la fixation du prix d'introduction. C'est une autre partie de l'essence de l'« événement de définition de coordonnée pré-introduction ».

---

## 8. La prochaine étape de vérification — Jalons clés du calendrier d'introduction

Non des déclencheurs de transaction. Des points d'observation montrant comment l'« événement de définition de coordonnée » progresse :

### 8.1 Jalons du calendrier

| Calendrier | Événement | Impact sur le scénario |
| --- | --- | --- |
| Mi-mai 2026 | Résultats T1 2026 de KakaoBank et Korea Investment Holdings | Formation des comparables coréens fintech |
| Fin mai–juin 2026 | Dépôt ou non du F-1 public | Non déposé = introduction T2 difficile → pression vers la zone basse |
| Juin 2026 | Si F-1 publié, vérification de la communication par segments | Variables 3, 4, 5 se referment comptablement |
| Juillet 2026 | Début du road show | Stade de décision de la variable 2 (classification de marché) |
| Août 2026 et après | Négociation du prix → finalisation du prix d'offre | Les sept variables s'agrègent en prix |

### 8.2 Sept éléments critiques dans le F-1/S-1

| Élément | Variable fermée |
| --- | --- |
| Décomposition des revenus Toss Securities + trajectoire trimestrielle de la marge opérationnelle | Variable 3 (cyclicité vs structuralité) |
| Marge sur contribution standalone Viva Republica | Variable 5 (trajectoire de rentabilité du corps applicatif) |
| Taux de capture publicitaire, cohortes clients, récurrence | Variable 4 (qualité de l'accélération publicitaire) |
| Taux de croissance du courtage par catégorie | Variable 4 (cause du ralentissement du courtage) |
| TPV Toss Payments / marge EBITDA | Décomposition du segment PG |
| Communication sur le plan d'introduction séparée de Toss Bank | Variable 6 |
| Ratio ADR + part des actions secondaires + conditions de lock-up | Variable 7 |
...
*Cet article est une analyse et un commentaire uniquement, et ne constitue pas un conseil en investissement. L'analyse en quatre points de la valorisation à l'introduction, le coût implicite des capitaux propres, les estimations SOTP, les cinq obstacles institutionnels d'une cotation américaine et les estimations de décote sont basés sur des rapports externes (Reuters, Yonhap, The Bell, Investing Korea) et des documents IR de la société, ainsi que sur les déductions de l'analyste fondées sur les analyses de Korea Investors Service ; le prix d'introduction réel, la classification de marché et les résultats futurs peuvent différer. Les quatre points de prix sont des outils montrant uniquement la « fourchette possible » — aucun point unique n'est traité comme un « scénario de référence », et ce refus constitue le principe analytique de cet article. L'hypothèse du modèle Gordon Growth (coût des capitaux propres de 9 %) est une ancre conservatrice ; les valeurs réelles peuvent varier. Les tickers cités sont illustratifs pour le cadre d'analyse, non des recommandations. Faites vos propres vérifications préalables et consultez des conseillers agréés avant toute décision d'investissement.*
