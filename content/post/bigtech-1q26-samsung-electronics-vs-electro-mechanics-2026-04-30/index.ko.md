...
- **빅테크의 메시지는 AI 수요 피로가 아니다. AI 투자 재가속이다.** AWS +28%, Azure +39~40%, Google Cloud +63% — 하이퍼스케일러 클라우드 세 곳이 *동시에* 성장을 가속하고 있다. 네 개사의 2026년 AI·클라우드 투자 합산 연환산 규모는 약 **6,500억 달러**다.
...
- **6~12개월 상대 선호도: 삼성전자 > 삼성전기.** 삼성전자 보유자는 홀드가 유리하고, 신규 매수는 눌림목을 노리는 편이 낫다. 삼성전기는 일부 차익 실현이 합리적이고, 현 가격에서 추격 매수는 비효율적이다.
...
주의 — 이 수치는 정확한 연간 투자지출 합계가 아니다. Microsoft는 단일 분기의 연환산이라 변동이 있고, Amazon의 "$200B AI 투자"에는 클라우드 외 항목도 포함된다. 이 숫자는 **수요 강도 지표**로 보면 된다. 정밀한 합계로 받아들이면 안 된다.
...
단기 차익 실현은 자연스럽지만, 구조적인 호재 소멸은 가능성이 낮다. 보유자는 유지가 유리하다. 신규 매수는 5일 또는 20일 이동평균 눌림목, 또는 추가 메모리 가격 데이터 확인 후가 낫다.
...
## 6. 매매 방향 — 같은 투자 숫자, 다른 결정

| 구분 | 행동 |
| --- | --- |
| **삼성전자 보유자** | 홀드 우선. HBM4 고객 확대와 2Q DS 마진이 확인되기 전까지 비중을 줄일 이유가 없다. |
| **삼성전자 신규 진입** | 급등 추격은 비효율적. 5일·20일 이동평균 눌림목 또는 메모리 가격 추가 데이터 확인 후 분할 매수. |
| **삼성전기 보유자** | 일부 차익 실현이 합리적. 전량 매도는 이르다. AI MLCC·FC-BGA 구조적 흐름은 살아 있다. |
| **삼성전기 신규 진입** | 지금 추격은 비효율적. 2Q 가이던스, 수주 언급, 영업마진 10% 근접 확인 후 진입. |

핵심: **동일한 빅테크 실적이 두 가지 다른 매매 판단을 지지한다.** AI 투자 *사이클*을 가져가고 싶다면 삼성전자다. AI 부품 *스토리*를 갖고 싶다면 가격을 기다려라.
...
*이 글은 투자 참고용 분석·해설이며, 투자 조언이 아닙니다. 의견은 예고 없이 바뀔 수 있습니다. 데이터 기준일: 2026년 4월 30일 KST.*
