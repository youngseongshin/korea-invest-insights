...
* **Citi TP ₩460,000.** Up from ₩300,000 (+53% revision). +61% upside vs. May 11 close of ₩285,500.
* **The Q1 print is the starting point.** Samsung Electronics 1Q26 consolidated OP ₩57.2tn. The DS (semiconductor) division alone produced OP ₩53.7tn at **65.7% margin**. That isn't a semiconductor business by financial profile; it's a monopoly platform.
* **Citi's core claim.** It's not just HBM (high-bandwidth memory for AI) pricing up. AI demand is absorbing memory-fab capacity broadly, **lifting commodity DRAM and NAND pricing along with HBM**. The argument is that this is a structural demand-shape change, not a transient cycle.
* **For ₩460,000 to clear.** The Q1-level earnings need to be "structurally sustained through 2H26" rather than a peak. The two verification variables are 2Q memory pricing trajectory and HBM4E customer qualifications.
...
Computers need two things: **compute devices** (CPU, GPU) and **storage / staging devices** (memory, storage).
...
**DRAM**: volatile memory; data disappears when power is off. Used for "data currently in use" — fast read / write. Goes into smartphones, PCs, servers.

**NAND**: non-volatile memory; data persists when power is off. Used in SSDs. Stores photos, videos, app data.

Samsung Electronics is **#1 globally in both DRAM and NAND** by market share. SK hynix is #2, Micron (US) is #3. These three account for the bulk of the global memory market.

### 1.2 HBM — the "oversized working desk" AI demanded

Standard DRAM is the "normal-sized desk." It's enough for web browsing, document work, gaming. **AI needs the desk to be enormous.** ChatGPT-class large language models need to manipulate hundreds of billions of parameters simultaneously.
...
**HBM generations**: HBM → HBM2 → HBM2E → HBM3 → HBM3E → **HBM4** → HBM4E. Higher generation = larger capacity, faster bandwidth, better power efficiency. Samsung Electronics **started HBM4 mass-production shipments in Q1** and plans **HBM4E sample supply in Q2**.
...
Samsung Electronics **started SOCAMM2 (2nd generation) mass-production shipments in Q1**. This form factor connects to NVIDIA's next-generation AI platforms (Vera Rubin and beyond).
...
**"AI token growth" — the Citi phrase** — is exactly this. As the number of tokens (word fragments) AI models process explodes, memory demand expands structurally.
...
The memory industry has a long-established pattern: **"prices rise → fabs expand → supply overshoots → prices crash."**
...
Citi's core argument is **"the conventional cycle may not work this time."** Two reasons:

**First, AI memory demand is structural, not cyclical.** Past memory-price cycles were driven by PC refresh cycles, smartphone adoption waves, datacenter expansion — temporary demand spikes. AI is different. Models keep getting bigger, users keep growing, AI agents consume more memory per "user" than humans did. Demand isn't "spikes and crashes" — it **keeps climbing**.

**Second, HBM absorbs fab capacity.** HBM is manufactured in the same fabs as standard DRAM, but each HBM stack consumes 3-4× more wafer surface than equivalent commodity DRAM. As AI demand pulls fab allocations into HBM, commodity DRAM output shrinks. So **it's not just HBM that gets expensive — commodity DRAM and NAND prices rise too.**
...
**65.7% operating margin** in context:
...
This isn't "good earnings" — it's a number that forces the question **"has the structure changed?"** In the conventional cycle, 65% margin is "peak of peak" and must mean-revert. If AI has structurally changed the demand shape, this margin can hold.
...
| Current cap / post-tax | **\~9.6×** |
| Target cap / post-tax | **\~15.5×** |
...
**Reading**: at the current price, Samsung Electronics trades at **\~9.6× annualized post-tax earnings**. For Citi's ₩460,000 to work, this multiple must rerate to **\~15.5×**. Moving from 9.6× to 15.5× requires a market mindset shift: "this isn't a peak, it's structurally sustainable."

**Compressed**: Citi TP ₩460,000 = **"earnings like Q1 will persist longer"** + **"so the market should pay a higher multiple."**
...
This translates to **broader memory demand than GPU demand**. GPUs compute; memory stores and shuttles the data that gets computed. As AI scales, you need more GPUs but you need *much more* memory. Like doing 1,000-digit multiplication — you need a calculator (GPU), but you need vastly more paper (memory).

### 4.2 Leg 2 — HBM4 / HBM4E / SOCAMM2 restructures product mix

The "mix" of memory Samsung sells is shifting. Standard DRAM used to dominate; now HBM and SOCAMM — **expensive, high-margin products** — make up an increasing share.
...
The most important leg. **It's not just HBM that gets expensive — commodity DRAM and NAND prices rise alongside.**

Why: Samsung / SK hynix / Micron run HBM and commodity DRAM on **shared fab lines**. As HBM demand explodes and fabs reallocate to HBM, commodity-DRAM output capacity shrinks. Shrinking supply lifts price.

**Why this matters for Samsung Electronics**: Samsung sells HBM, commodity DRAM, NAND, and eSSD (high-performance server SSDs). When all of these prices rise simultaneously, the earnings story isn't "HBM monoculture" — it's **portfolio-wide lift**. This is Citi's "ASP across-the-board upward revision" thesis.
...
|---|---|---|---|
| 1 | **Samsung Electronics** | Direct | The subject. Portfolio-wide memory price lift |
| 2 | **SK hynix** | Direct | HBM #1. Memory price lift is sector-wide positive |
...
**Read**: Samsung Electronics > SK hynix > Samsung Electro-Mechanics > 2nd-tier substrate / materials. Directness diminishes down the chain. The reflex "memory prices up → buy all AI names" is a logical leap.
...
Citi lifted Samsung Electronics' target to ₩460,000. The report's substance is not "Samsung is a great stock." It's that **the 30-year-old belief — 'memory prices peak and crash' — may be wrong this time**, because AI has reshaped memory demand and HBM is pulling fab capacity to the point that commodity-memory prices rise alongside.
...
**Q: How did Citi arrive at ₩460,000?**
...
**Q: Is the memory cycle actually over?**
...
**Q: Why do HBM and commodity DRAM prices rise together?**
...
**Q: Samsung Electronics vs. SK hynix — which is better?**
...
**Q: At \~$1.2 trillion market cap, isn't it already expensive?**
...
**Q: When is Citi's thesis verified fastest?**
